...
foreach ($articles as $idArticle => $article) {
    ...
    $query = "UPDATE article AS a
              ...
              SET a.nom = :nom, a.description = :description
              WHERE lha.listeDeSouhait_idlisteDeSouhait = :idliste AND lha.article_idarticle = :idarticle";
    $statement = $pdo->prepare($query);
    $statement->execute([':nom' => $nomArticle, ':description' => $descriptionArticle, ':idliste' => $idListe, ':idarticle' => $idArticle]);
}
